...
thumb_dir = Path("assets/thumbs2019")
...
<head>
<meta charset="UTF-8">
<title>KALD 2019 Conference Gallery</title>
...
.btn,.searchPanel button{{display:inline-block;background:#0b4f8a;color:white;text-decoration:none;padding:12px 18px;border-radius:10px;font-weight:700;border:0;cursor:pointer}}
...
.gallery{{max-width:1150px;margin:auto;padding:25px;display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:16px}}
...
.photo img{{width:100%;height:180px;object-fit:cover;display:block;cursor:pointer}}
...
@media(max-width:760px){{nav{{padding:12px 14px;flex-wrap:wrap}}.brand{{width:100%;justify-content:center;margin-bottom:8px}}.nav-links{{width:100%;justify-content:center;flex-wrap:wrap;gap:12px}}.nav-links a{{font-size:14px}}.gallery{{grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:12px;padding:12px}}.photo img{{height:135px}}.viewer .topbar{{display:block;text-align:center}}.viewer button{{font-size:12px;padding:9px 10px}}}}
</style>
</head>
<body>
...
<section class="hero"><h1>2019 Conference Gallery</h1><p>476 optimized photo records from KALD event archive dated 27 November 2019.</p></section>
...
function updateViewer(){{const img=visiblePhotos[current].querySelector("img");document.getElementById("viewerImg").src=img.src.replace("assets/thumbs2019/","assets/medium2019/");document.getElementById("viewerImg").style.transform="scale("+zoom+")";document.getElementById("counter").textContent="Photo "+(current+1)+" of "+visiblePhotos.length+" - "+img.alt;}}
...
<footer><p>KALD Digital Archive Center | 2019 Conference Gallery</p></footer>
</body>
</html>'''

Path("photos-2019.html").write_text(html, encoding="utf-8")
print(f"Generated photos-2019.html with {len(photos)} photos.")
